...
function onMessage(evt) {
	...
	if (obj.hasOwnProperty('studios')) {
		...
		if (obj.studios.length <= 0) { //VALIDAR ESSA PARTE
			$('<li>').text("N„o h· est˙dios disponÌveis").prependTo("#listaEstudios");
		}
		...
		if (obj.scores.length <= 0) { //VALIDAR ESSA PARTE
			$('#listaPartEst').append($('<li/>', {'text' : "N√£o h√° partituras dispon√≠veis"}));
			$('#listaPart').append($('<li/>', {'text' : "N√£o h√° partituras dispon√≠veis"}));
		} else {
